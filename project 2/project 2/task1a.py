...
# tokenized the texts and turn it into vector form 
...
# calculate the cosine similarity of the texts
def get_cosine_sim(*strs): 
    vectors = [t for t in get_vectors(*strs)]
    return cosine_similarity(vectors)

# calculate the price similarity
...
for i in range(len(data_small)):
    ...
    # get the description similarity
    ...
    des_similarity.append(des_sim)
     ...
     # get the manufacturer similarity
    ...
    # calculate the final similarity score between the products
...
'''
# Find the inportance of each feature in determining whether it is a match
...
for i in range(len(data_small)):
    ...
    for j in range(len(truth_small)):
        if data_small['idAmazon'][i]==truth_small['idAmazon'][j] and data_small['idGoogleBase'][i]==truth_small['idGoogleBase'][j] and matched == 0:
            check_match.append('match')
            matched = 1
    if not matched:
        check_match.append('not match')

data_small['matched'] = check_match
...
# Find the matches which exceed the threshold
for i in range(len(data_small)):
